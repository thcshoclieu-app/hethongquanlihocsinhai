...
export default function Login() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    try {
      ...
      navigate(profile.role === 'PARENT' ? ROUTES.PARENT_DASHBOARD : ROUTES.DASHBOARD);
    } catch (err: any) {
  ...
  return (
    <Card className="w-full shadow-lg border-0 ring-1 ring-slate-200 dark:ring-slate-800">
      <CardHeader className="space-y-2 text-center pb-8">
        ...
        <CardDescription>Đăng nhập vào hệ thống quản lý học sinh</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Button type="submit" className="w-full h-11 text-base mt-2" disabled={loading}>
            {loading ? <Spinner className="mr-2 text-white" size={16} /> : null}
            Đăng nhập
          </Button>
        </form>
      </CardContent>
      <CardFooter className="flex flex-col space-y-4">
        <div className="relative w-full">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-white dark:bg-slate-950 px-2 text-slate-500">Hoặc tiếp tục với</span>
          </div>
        </div>
