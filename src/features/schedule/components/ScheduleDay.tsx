...
export function ScheduleDay({ date, sessions, isLoading, onSessionClick }: ScheduleDayProps) {
  const hours = Array.from({ length: 15 }, (_, i) => i + 7); // 7:00 to 21:00

  const daySessions = sessions.filter(session => isSameDay(new Date(session.date), date));
  ...
  return (
    <div className="h-full flex flex-col relative overflow-hidden">
      <div className="grid grid-cols-[80px_1fr] border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shrink-0 sticky top-0 z-10">
        <div className="py-3 px-2 text-center text-xs font-medium text-slate-500 border-r border-slate-200 dark:border-slate-800">
          Giờ
        </div>
        <div className="py-2 px-4 border-r border-slate-200 dark:border-slate-800 bg-blue-50/50 dark:bg-blue-900/10">
          <div className="text-xs font-medium text-slate-500">{format(date, 'EEEE', { locale: vi })}</div>
          <div className="text-lg font-semibold mt-0.5 text-blue-600 dark:text-blue-400">
            {format(date, 'dd/MM')}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto relative">
        <div className="grid grid-cols-[80px_1fr] min-h-max relative" style={{ height: `${hours.length * 60}px` }}>
          {/* Time axis */}
          ...
          {/* Day column */}
          <div className="border-r border-slate-200 dark:border-slate-800 relative bg-blue-50/20 dark:bg-blue-900/5">
            ...
            {daySessions.map((session, index) => {
              ...
              return (
                <div 
                  ...
                  className={`absolute left-2 right-2 md:w-1/2 md:left-[calc(50%*${index%2})] rounded-lg p-3 overflow-hidden text-sm cursor-pointer shadow-sm border transition-shadow hover:shadow-md ${
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="font-bold">{session.classId}</div>
                    <div className="text-xs font-medium px-2 py-0.5 rounded bg-white/50 dark:bg-black/20">
                      {session.roomId}
                    </div>
                  </div>
                  <div className="text-xs opacity-90 mt-1">{session.startTime} - {session.endTime}</div>
                  <div className="text-xs opacity-90 mt-1 flex items-center gap-1">
                    <span className="w-2 h-2 rounded-full bg-current opacity-50"></span>
                    Giáo viên: {session.teacherId}
                  </div>
                </div>
              );
            })}
